        ...
        <nav className="space-y-1">
          {navItems.map(n => (
            <Link
              ...
              className="group flex items-center gap-3 rounded-md px-3 py-2.5 text-xs text-muted-foreground transition hover:bg-secondary hover:text-foreground"
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-6 border-t border-border bg-background/95 px-1 py-2 backdrop-blur-xl lg:hidden">
        {navItems.map(n => (
          <Link
            ...
            to={n.to}
            activeOptions={{ exact: n.to === "/" }}
            className="flex min-w-0 flex-col items-center gap-1 py-1 text-[9px] text-muted-foreground"
